...
- `BMS-REQ-118` / `cooling_response_ms`: baseline `410.0` (pass), current `612.0` (fail), threshold `<= 500.0`.
...
- cooling_response_ms for BMS-REQ-118 was 612, outside <= 500.
- Mapped affected files: src/bms/thermal_manager.c.
- cooling_response_ms materially worsened from 410.0 to 612.0.
...
- `BMS-SIL-FASTCHARGE-01` / `max_cell_temp_c`: min `44.2`, max `52.8`, last `52.8` C, points `4`.
