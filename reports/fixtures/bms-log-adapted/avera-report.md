...
- `BMS-HIL-FASTCHARGE-07`: baseline `passed`, current `failed`, component BMS Thermal Control.
...
- `BMS-REQ-118` / `cooling_response_ms`: baseline `412.0` (pass), current `638.0` (fail), threshold `<= 500.0`.
...
- cooling_response_ms for BMS-REQ-118 was 638, outside <= 500.
- Mapped affected files: src/bms/thermal_manager.c.
- cooling_response_ms materially worsened from 412.0 to 638.0.
- max_cell_temp_c materially worsened from 47.1 to 52.8.
...
- `BMS-HIL-FASTCHARGE-07` / `max_cell_temp_c`: min `44.1`, max `52.8`, last `52.8` C, points `4`.
